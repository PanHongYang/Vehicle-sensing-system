...
struct Header {
  ...
  unsigned int id : 29;       ///< CAN ID (11 or 29 bits valid, depending on
                              ///< is_extended member
  ...
  bool isValid() const { return id < (is_extended ? (1 << 29) : (1 << 11)); }
  ...
  Header() : id(0), is_error(0), is_rtr(0), is_extended(0) {}

  Header(unsigned int i, bool extended, bool rtr, bool error)
      : id(i),
        is_error(error ? 1 : 0),
        is_rtr(rtr ? 1 : 0),
        is_extended(extended ? 1 : 0) {}
};
...
struct Frame : public Header {
  boost::array<unsigned char, CANFD_MAX_DLEN>
      data;  ///< array for CANFD_MAX_DLEN data bytes with bounds checking
  unsigned char len;  ///< len of data

  /** check if frame header and length are valid*/
  bool isValid() const { return (len <= data.size()) && Header::isValid(); }
  /**
   * constructor with default parameters
   * @param[in] i: CAN id, defaults to 0
   ...
   */
  Frame() : Header(), len(0) {}
  Frame(const Header &h, unsigned char l = 0) : Header(h), len(l) {}
};

/** extended error information */
class State {
 public:
  enum DriverState { closed, open, ready } driver_state;
...
class StateInterface {
 public:
  ...
  /**
   * acquire a listener for the specified delegate, that will get called for all
   * state changes
   *
   * @param[in] delegate: delegate to be bound by the listener
   * @return managed pointer to listener
   */
  virtual StateListener::Ptr createStateListener(
...
class CommInterface {
 public:
   ...
   * @return managed pointer to listener
   */
  virtual FrameListener::Ptr createMsgListener(
      const FrameDelegate &delegate) = 0;

  /**
   * acquire a listener for the specified delegate, that will get called for
   * messages with demanded ID
   *
   ...
   */
  virtual FrameListener::Ptr createMsgListener(
      const Frame::Header &, const FrameDelegate &delegate) = 0;
...
class DriverInterface : public CommInterface, public StateInterface {
 public:
  ...
  virtual bool translateError(unsigned int internal_error,
                              std::string &str) = 0;
  ...
  virtual std::string GetDeviceName() const { return device_; }
...
#define LOG(log)                                                           \
  {                                                                        \
    boost::mutex::scoped_lock _cout_lock(_cout_wrapper::get_cout_mutex()); \
