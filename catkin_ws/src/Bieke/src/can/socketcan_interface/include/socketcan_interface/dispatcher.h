...
class SimpleDispatcher {
 ...
 protected:
  class DispatcherBase : boost::noncopyable {
    class GuardedListener : public Listener {
     ...
     public:
      GuardedListener(boost::shared_ptr<DispatcherBase> g,
   ...
   public:
    ...
    void dispatch_nolock(const Type &obj) const {
      for (typename std::list<Listener *>::const_iterator it =
               listeners_.begin();
    ...
    static typename Listener::Ptr createListener(
        boost::shared_ptr<DispatcherBase> dispatcher,
        const Callable &callable) {
 ...
 public:
  ...
  size_t numListeners() { return dispatcher_->numListeners(); }
...
class FilteredDispatcher : public SimpleDispatcher<Listener> {
  ...
  boost::unordered_map<K, boost::shared_ptr<typename BaseClass::DispatcherBase>,
                       Hash>
      filtered_;

 public:
  using BaseClass::createListener;
  typename Listener::Ptr createListener(
