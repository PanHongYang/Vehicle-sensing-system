...
class SocketCANInterface
 ...
 public:
  SocketCANInterface() : loopback_(false), sc_(-1) {}
  ...
  virtual bool init(const std::string& device, bool loopback) {
    ...
    if (s.driver_state == State::closed) {
      ...
      if (sc < 0) {
        setErrorCode(
            boost::system::error_code(sc, boost::system::system_category()));
        return false;
      }
      ...
      ret = setsockopt(sc, SOL_CAN_RAW, CAN_RAW_ERR_FILTER, &err_mask,
                       sizeof(err_mask));
      ...
      if (loopback_) {
        ...
        ret = setsockopt(sc, SOL_CAN_RAW, CAN_RAW_RECV_OWN_MSGS, &recv_own_msgs,
      ...
      setsockopt(sc, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &canfd_on,
                 sizeof(canfd_on));
  ...
  int getInternalSocket() { return sc_; }
 ...
 protected:
  ...
  virtual void triggerReadSome() {
    ...
    socket_.async_read_some(boost::asio::buffer(&frame_, sizeof(frame_)),
  ...
  virtual bool enqueue(const Frame& msg) {
    ...
    canfd_frame frame = {0};
    frame.can_id = msg.id | (msg.is_extended ? CAN_EFF_FLAG : 0) |
                   (msg.is_rtr ? CAN_RTR_FLAG : 0);
    ;
    frame.len = msg.len;

    for (int i = 0; i < frame.len; ++i) frame.data[i] = msg.data[i];

    boost::system::error_code ec;
    boost::asio::write(socket_, boost::asio::buffer(&frame, sizeof(frame)),
  ...
  void readFrame(const boost::system::error_code& error) {
    if (!error) {
      input_.len = std::max((__u8)input_.data.size(), frame_.len);
      for (int i = 0; i < input_.len; ++i) {
        input_.data[i] = frame_.data[i];
      }
      for (int i = input_.len; i < input_.data.size(); ++i) {
        input_.data[i] = 0;
      }
      ...
      if (frame_.can_id & CAN_ERR_FLAG) {  // error message
        ...
        input_.id =
            frame_.can_id & (input_.is_extended ? CAN_EFF_MASK : CAN_SFF_MASK);
        input_.is_error = 0;
