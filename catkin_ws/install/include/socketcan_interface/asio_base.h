...
class AsioDriver : public DriverInterface {
  typedef FilteredDispatcher<const unsigned int, CommInterface::FrameListener>
      FrameDispatcher;
 ...
 protected:
  ...
  void dispatchFrame(const Frame &msg) {
    strand_.post(boost::bind(&FrameDispatcher::dispatch, &frame_dispatcher_,
                             msg));  // copies msg
  }
  ...
  AsioDriver() : strand_(io_service_), socket_(io_service_) {}
 ...
 public:
  ...
  virtual FrameListener::Ptr createMsgListener(const Frame::Header &h,
