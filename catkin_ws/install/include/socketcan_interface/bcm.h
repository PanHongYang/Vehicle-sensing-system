...
class BCMsocket {
  ...
  struct Message {
    ...
    bcm_msg_head &head() { return *(bcm_msg_head *)data; }
    template <typename T>
    void setIVal2(T period) {
      long long usec =
          boost::chrono::duration_cast<boost::chrono::microseconds>(period)
              .count();
    ...
    bool write(int s) { return ::write(s, data, size) > 0; }
    ~Message() {
      delete[] data;
      data = 0;
      size = 0;
 ...
 public:
  ...
  bool startTX(DurationType period, Header header, size_t num, Frame *frames) {
    ...
    for (size_t i = 0; i < num; ++i) {  // msg nr
      head.frames[i].can_dlc = frames[i].len;
